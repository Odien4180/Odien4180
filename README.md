## Hi there 👋

<!--
**Odien4180/Odien4180** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->

<!-- GITHUB_ACTIVITY_REPORTER:START -->

## Development Pulse

> 보고 기간: 2026-07-28 ~ 2026-07-29 · 업데이트 2026-07-29 08:14 KST

<p align="center">
  <img src="./generated/activity-dashboard.svg" alt="개발 활동 대시보드" width="100%" />
</p>

| 공개 저장소 | 공개 커밋 | 비공개 저장소 | 비공개 커밋 |
| :---: | :---: | :---: | :---: |
| **3** | **27** | **2** | **6** |

### 공개 프로젝트

#### 이번 활동 요약

> 활발한 자동화 및 유지보수 중심의 개발 활동 진행

- Github-activity-reporter 프로젝트에서 자동화 및 코드 유지보수에 집중, 다수의 커밋과 리뷰 수행
- Odien4180 저장소에서는 개인 설정 및 코드 정비에 중점
- MVP 프로젝트는 초기 기능 구현에 집중하여 여러 커밋 진행

#### [Odien4180/Github-activity-reporter](https://github.com/Odien4180/Github-activity-reporter)

> 사용자 깃 활동 리포팅 환경 자동 구성

자동화와 유지보수 작업이 주로 이루어졌으며, 리뷰 활동도 함께 진행되어 개발의 안정성을 높임.

**활동:** 커밋 15건 · 작성한 리뷰 2건

<sub>주요 언어: C#</sub>

#### [Odien4180/Odien4180](https://github.com/Odien4180/Odien4180)

개인 설정 및 정비 위주로 다수의 커밋이 이루어짐.

**활동:** 커밋 9건

<sub>주요 언어: HTML</sub>

#### [Odien4180/MVP](https://github.com/Odien4180/MVP)

> 유니티에서 UI 구현에 사용하기 위한 MVP 패턴 구현

MVP 기능의 초기 구현에 집중하여 기초 개발이 활발히 진행됨.

**활동:** 커밋 3건

<sub>주요 언어: C#</sub>

### 비공개 활동

최근 보고 기간 동안 2개의 비공개 저장소에서 활동했습니다.

**집계:** 커밋 6건 · 활동한 날 1건

<sub>GitHub Activity Reporter가 자동으로 생성한 요약입니다.</sub>

<!-- GITHUB_ACTIVITY_REPORTER:END -->
